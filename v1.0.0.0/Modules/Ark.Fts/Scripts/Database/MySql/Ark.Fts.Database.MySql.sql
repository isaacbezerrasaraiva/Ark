...
create table FtsIncrement
(
	IdTable smallint,
    TableName varchar(32),
    constraint Pk_FtsIncrement primary key (IdTable)
);
...
alter table FtsIncrement 
	add constraint Uk_FtsIncrement1 unique key (TableName);
    
alter table FtsIncrementByDomain 
	add constraint Fk_FtsIncrementByDomain1 foreign key (IdTable) references FtsIncrement (IdTable),
...
alter table FtsIncrementByDomainMaster 
	add constraint Fk_FtsIncrementByDomainMaster1 foreign key (IdTable) references FtsIncrement (IdTable),
...
alter table FtsIncrementByBranch 
	add constraint Fk_FtsIncrementByBranch1 foreign key (IdTable) references FtsIncrement (IdTable),
...
alter table FtsIncrementByBranchMaster 
	add constraint Fk_FtsIncrementByBranchMaster1 foreign key (IdTable) references FtsIncrement (IdTable),
