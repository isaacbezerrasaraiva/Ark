...
namespace Ark.Lib.Client
{
    public static class LibConfigurationClient
    {
        #region Consts

        private const string ARK_LIB_CLIENT_XML = "Ark.Lib.Client.xml";
        ...
        public static void Load()
        {
            ...
            if (File.Exists(Path.Combine(LibDirectory.Root.Dat.Path, ARK_LIB_CLIENT_XML)) == false)
                Save();
            ...
            xml.Open(Path.Combine(LibDirectory.Root.Dat.Path, ARK_LIB_CLIENT_XML));
        ...
        public static void Save()
        {
            ...
            XmlNode xmlNodeRoot = xml.WriteRoot("Ark.Lib.Client");

            SaveDynamicXml(xml, xmlNodeRoot);

            xml.Save(Path.Combine(LibDirectory.Root.Dat.Path, ARK_LIB_CLIENT_XML));
        ...
        {
            dynamicXml.LoadDynamicXml(xml, "/Ark.Lib.Client/DynamicXml");
        }
